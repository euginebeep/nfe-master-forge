...
export async function isSuperDev(): Promise<boolean> {
  const { data, error } = await rpc("is_super_dev", {});
  if (error) return false;
  return Boolean(data);
}

export async function isGhostActive(): Promise<boolean> {
  const { data, error } = await rpc("is_ghost_mode", {});
  if (error) return false;
  return Boolean(data);
}

export async function startGhost(targetCompanyId: string) {
  const { data, error } = await rpc("start_ghost_session", {
...
export async function stopGhost() {
  const { error } = await rpc("stop_ghost_session", {});
...
export async function readGhostAudit(opts: {
  ...
  const { data, error } = await rpc("read_ghost_audit", {
