...
export default function QuarentenaPage() {
  ...
  const [selectedLote, setSelectedLote] = useState<(LocalEstoqueLote & { item?: LocalItem }) | null>(null);
  ...
  useEffect(() => {
    const loadLotes = () => {
      setLoading(true);
      const data = LocalDb.getCollection<LocalEstoqueLote>('estoque_lotes');
      const itens = LocalDb.getCollection<LocalItem>('itens');
      
      // Filter only QUARENTENA status
      const quarentena = data
        .filter(lote => lote.status === 'QUARENTENA')
        .map(lote => ({
          ...lote,
  ...
  const handleAction = (lote: LocalEstoqueLote & { item?: LocalItem }, type: 'liberar' | 'bloquear') => {
  ...
  const confirmAction = () => {
    ...
    // Get current lote and update it
    const currentLote = LocalDb.getById<LocalEstoqueLote>('estoque_lotes', selectedLote.id);
    if (currentLote) {
      const updated = {
        ...currentLote,
        status: newStatus,
        observacoes_qc: observacoes || currentLote.observacoes_qc,
      };
      LocalDb.update('estoque_lotes', selectedLote.id, updated);
  ...
  // Check if lote has validated COA
  const hasValidatedCOA = (loteId: string) => {
    const docs = LocalDb.query<any>('lote_documentos', d => d.lote_id === loteId);
  ...
  const totalItens = new Set(lotes.map(l => l.item_id)).size;
  const lotesComCOA = lotes.filter(l => hasValidatedCOA(l.id)).length;
  ...
  return (
    <div>
      <PageHeader
        title="Quarentena"
        description="Lotes aguardando liberação para o estoque oficial"
        icon={ShieldAlert}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <Card className="border-success/30 bg-success/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-success">{lotesComCOA}</div>
            <p className="text-xs text-muted-foreground">com COA validado</p>
          </CardContent>
        </Card>
        ...
        <Card className="border-warning/30 bg-warning/5">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-warning">{lotesSemCOA}</div>
            <p className="text-xs text-muted-foreground">sem documentação</p>
          </CardContent>
        </Card>
      </div>
      ...
      <DataTable
        ...
        emptyMessage="Nenhum lote em quarentena"
      />

      {/* Confirmation Dialog */}
      <Dialog open={!!selectedLote && !!actionType} onOpenChange={() => {
        setSelectedLote(null);
        setActionType(null);
      }}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {actionType === 'liberar' ? 'Liberar Lote para Estoque' : 'Bloquear Lote'}
            </DialogTitle>
            <DialogDescription>
              {actionType === 'liberar' 
                ? 'Este lote será movido para o estoque oficial e ficará disponível para uso.'
                : 'Este lote será bloqueado e não poderá ser utilizado.'}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="bg-muted p-4 rounded-lg space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-medium">{selectedLote.numero_lote}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Produto:</span>
                  <span className="font-medium">{selectedLote.item?.descricao_interna}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Quantidade:</span>
                  <span>
                    {formatNumber(selectedLote.quantidade_interna, 2)} {selectedLote.item?.unidade_interna || selectedLote.unidade_original}
                  </span>
                </div>
                {!hasValidatedCOA(selectedLote.id) && actionType === 'liberar' && (
                  <div className="flex items-center gap-2 text-warning mt-2 p-2 bg-warning/10 rounded">
                    <AlertTriangle className="h-4 w-4" />
                    <span className="text-sm">Este lote não possui COA validado</span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  placeholder="Motivo da liberação/bloqueio..."
                  value={observacoes}
