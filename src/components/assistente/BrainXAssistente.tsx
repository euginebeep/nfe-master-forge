...
import { extractThrownEdgeError, invokeEdge } from '@/lib/edge-invoke';
...
function BrainXAssistentePanel() {
  ...
  const enviar = useCallback(async (textoOverride?: string) => {
    ...
    try {
      ...
      const msg = await extractThrownEdgeError(e, 'Ops, tive um problema de conexão. Tente novamente em instantes.');
      setMensagens(prev => [...prev, {
        id: crypto.randomUUID(),
        role: 'assistant',
        content: msg,
