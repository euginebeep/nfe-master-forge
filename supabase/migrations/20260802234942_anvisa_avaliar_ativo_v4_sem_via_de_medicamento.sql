-- Substitui o texto 'proxima_via' que sugeria fitoterapico. Agora o motor
-- recebe company_id opcional e devolve so as vias que a planta pode seguir.
...
BEGIN
  ...
  IF v_id IS NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      'acao_da_rt','Confirmar em item_anvisa_vinculo qual constituinte autorizado '
        ...
        || 'serve para suplemento alimentar — confirmar vinculo IDENTIFICA, nao autoriza.',
  ...
  IF COALESCE(v_orfao,false) THEN
    RETURN jsonb_build_object('status','PENDENTE_VERIFICACAO','ativo',p_nome,
      ...
      'responsavel','plataforma',
      'acao','Determinar na fonte oficial o que ocorreu. NAO e decisao de RT: a '
        || 'regra da ANVISA nao pertence a tenant. O tenant e informado do resultado.',
