-- CORRECAO DE PRINCIPIO. A mensagem anterior dizia "exige decisao da RT".
-- Errado: se a ANVISA renomeou, redesignou a cepa ou desautorizou um
-- constituinte, isso e fato sobre A REGRA. Nenhuma RT de tenant decide isso.
-- E pendencia da PLATAFORMA: alguem le a fonte oficial e determina.
-- A RT e INFORMADA do resultado; nao e consultada sobre a lei.
...
COMMENT ON VIEW public.v_anvisa_constituintes_orfaos IS
  ...
  'PENDENCIA DA PLATAFORMA — determinar na fonte oficial se houve renomeacao, '
  'redesignacao de cepa ou desautorizacao. NAO e decisao de RT de tenant: '
  'regra da ANVISA nao pertence a tenant e nao depende de aprovacao. '
  'Ate a determinacao, o motor responde PENDENTE_VERIFICACAO e o tenant e '
  'INFORMADO — nao consultado.';
...
DECLARE
  v_c anvisa_constituintes%ROWTYPE; v_id uuid;
...
BEGIN
  ...
  IF FOUND THEN
    RETURN jsonb_build_object('status','NAO_AUTORIZADO','ativo',p_nome,
      ...
      'fonte','anvisa_ingredientes_nao_autorizados');
  END IF;
  ...
  IF v_id IS NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      -- Aqui SIM a RT atua: dizer QUAL constituinte este insumo representa
      -- e decisao de aplicacao, do tenant. Nunca sobre o limite em si.
      'responsavel','rt_do_tenant_confirma_vinculo',
      'acao_da_rt','Confirmar em item_anvisa_vinculo qual constituinte autorizado '
        || 'este insumo representa, com teor. Se nenhum representa, o insumo nao serve.',
  ...
  IF COALESCE(v_orfao,false) THEN
    RETURN jsonb_build_object('status','PENDENTE_VERIFICACAO','ativo',p_nome,
      ...
      'responsavel','plataforma',
      'acao','Determinar na fonte oficial (DataLegis / painel) o que ocorreu. '
        || 'NAO e decisao de RT: a regra da ANVISA nao pertence a tenant. '
        || 'O tenant e informado do resultado.',
      'orfao_do_painel', true, 'dias_sem_aparecer', v_dias,
      'ultima_vez_no_painel', v_c.sincronizado_em, 'fonte','v_anvisa_constituintes_orfaos');
  END IF;
  ...
  IF v_c.limite_parse_status IS DISTINCT FROM 'ok' THEN
    ...
    v_motivo := 'Limite oficial nao parseado ('||COALESCE(v_c.limite_parse_status,'null')
             ...
             || 'Parsear a norma e trabalho da plataforma, nao da RT.';
