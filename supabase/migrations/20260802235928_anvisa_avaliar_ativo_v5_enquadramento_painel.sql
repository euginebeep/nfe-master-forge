...
BEGIN
  ...
  IF v_id IS NULL THEN
    ...
    RETURN jsonb_build_object(
      ...
      'motivo','Nao localizado na base oficial sincronizada do painel da ANVISA '
        ...
        || 'inclui os autorizados por Resolucao Especifica, alem da IN 28/2018 e '
        || 'suas alteradoras. Lista positiva: ausencia nao autoriza uso.',
      ...
      'acao_da_rt','Confirmar em item_anvisa_vinculo qual constituinte autorizado '
        || 'este insumo representa, com teor. Confirmar vinculo IDENTIFICA, nao autoriza.',
      'acao_da_plataforma','Antes de tratar como definitivo, conferir a pagina de '
        || 'ingredientes/REs da ANVISA: RE publicada apos a ultima sync ainda nao '
        || 'esta nesta base.',
  ...
  ELSE
    v_status := 'APROVADO';
    v_motivo := 'Consta da base oficial sincronizada e dose dentro do limite. '
             || 'Oficial: "'||v_txt||'".';
  END IF;
